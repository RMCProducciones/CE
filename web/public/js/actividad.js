...
app.controller('gestionDocumentoSoporteActividadCtrl', ['$scope', '$location', function($scope, $location) {
	...
	$scope.anularSoporteActividad = function() { 
		...
		window.location.replace($scope.rutaServidor  + "gestion-empresarial/desarrollo-empresarial/concurso/actividades/" + $scope.idActividad + "/documentos-soporte/" + $scope.idActividadSoporteActivo + "/borrar");
...
app.controller('gestionActividadCtrl', ['$scope', '$http', function($scope, $http) {
	...
	$scope.eliminarActividad = function(idActividad, consecutivo){
		...
		$http.get($scope.rutaServidor + "gestion-empresarial/desarrollo-empresarial/concurso/actividades/" + $scope.idActividad + "/eliminar")
